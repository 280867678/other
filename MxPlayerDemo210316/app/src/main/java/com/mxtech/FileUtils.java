...
//        }
//    };
//    private static final char[] kIllegalFileChars = {'/', '\n', '\r', '\t', 0, '\f', '`', '?', '*', '\\', '<', '>', '|', '\"', ':'};
//
...
//    }
    ...
    public static String[] list(File file) {
        return App.native_initialized ? listImpl(file.getPath()) : file.list();
    }
...
//    }
    ...
    public static File[] listFiles(File file) {
        return App.native_initialized ? filenamesToFiles(file, list(file)) : file.listFiles();
    }
...
//    }
//
//    public static boolean isFileScheme(@Nullable String scheme) {
//        return scheme == null || MediaListFragment.TYPE_FILE.equals(scheme);
//    }
//
...
//    }
//
//    public static OutputStream openOutputStream(File file, String mime) throws IOException {
//        String parent;
//        DocumentFile parentDoc;
//        try {
//            OutputStream os = new FileOutputStream(file);
//            Log.i(TAG, "Opened FileOuputStream for [" + file + "].");
//            return os;
//        } catch (IOException e) {
//            Log.w(TAG, e + " is thrown while opening [" + file + "] using FileOutputStream for writing.");
//            if (Build.VERSION.SDK_INT >= 21) {
//                DocumentFile doc = DocumentTreeRegistry.fromFile(file);
//                if (doc == null && (parent = file.getParent()) != null && (parentDoc = DocumentTreeRegistry.fromPath(parent)) != null) {
//                    doc = parentDoc.createFile(mime, file.getName());
//                    if (doc != null) {
//                        Log.i(TAG, "New document file [" + doc.getUri() + "] is created.");
//                    } else {
//                        Log.w(TAG, "Can't create document file with the name of '" + file.getName() + "' under " + parentDoc.getUri() + '.');
//                    }
//                }
//                if (doc != null) {
//                    OutputStream os2 = MXApplication.context.getContentResolver().openOutputStream(doc.getUri());
//                    Log.i(TAG, "Opened OutputStream from ContentResolver for [" + doc.getUri() + "].");
//                    return os2;
//                }
//            }
//            throw e;
...
//    }
//
//    public static boolean performDirectoryWritingTest(@NonNull File file) {
//        try {
//            File.createTempFile(".mxvp", null, file).delete();
//            return true;
//        } catch (IOException e) {
//            Log.w(TAG, "[" + file.getPath() + "] is NOT a writable directory - " + e + " is thrown while creating a temporary file.");
//            if (Build.VERSION.SDK_INT >= 21) {
//                DocumentFile doc = DocumentTreeRegistry.fromFile(file);
//                if (doc != null) {
//                    boolean writable = doc.canWrite();
//                    if (writable) {
//                        Log.i(TAG, "DocumentFile [" + doc.getUri() + "] is writable.");
//                        return writable;
//                    }
//                    Log.w(TAG, "DocumentFile [" + doc.getUri() + "] is NOT writable.");
//                    return writable;
//                }
//                Log.w(TAG, "DocumentFile is not found for [" + file + "].");
//            }
//            return false;
//        }
//    }
...
//        return !StringUtils.containsAnyOf(name, kIllegalFileChars);
//    }
//
