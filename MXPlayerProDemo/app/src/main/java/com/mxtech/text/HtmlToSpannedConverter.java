...
class HtmlToSpannedConverter implements ContentHandler {
    ...
    private void handleStartTag(String tag, Attributes attributes) {
        if (!tag.equalsIgnoreCase(Authorizer.PARAM_BRAND)) {
    ...
    private void handleEndTag(String tag) {
        if (tag.equalsIgnoreCase(Authorizer.PARAM_BRAND)) {
            ...
            end(this.mSpannableStringBuilder, Monospace.class, new android.text.style.TypefaceSpan("monospace"));
        } else if (tag.equalsIgnoreCase("a")) {
    ...
    private static void endFont(SpannableStringBuilder text) {
        ...
        if (where != len) {
            ...
            if (f.mFace != null) {
                text.setSpan(new android.text.style.TypefaceSpan(f.mFace), where, len, 33);
            }
        }
    }
