...
public class SubtitleSyncBar implements IFloatingBar, View.OnClickListener, TextWatcher {
    private static final int SUBTITLE_SYNC_UNIT = 100;
    ...
    public SubtitleSyncBar(ViewGroup parent, LayoutInflater layoutInflater, SubView subView, IPlayer player) {
        this.theView = subView;
        this._player = player;
        this._layout = (ViewGroup) layoutInflater.inflate(R.layout.subtitle_sync_bar, parent).findViewById(R.id.subtitle_sync_bar);
    ...
    public void onTextChanged(CharSequence s, int start, int before, int count) {
        try {
            this.theView.setSync((int) Math.round(Double.parseDouble(s.toString()) * 1000.0d));
    ...
    private void changeSync(int deltaMs) {
        this.theView.setSync(this.theView.getSync() + deltaMs);
        updateSyncText();
    }

    private void updateSyncText() {
        this._text.setText(this._formatter.format(this.theView.getSync() / 1000.0d));
    }
