...
public final class PlayService extends Service implements Player.Client, PlayLister.Listener, OrderedSharedPreferences.OnSharedPreferenceChangeListener, AudioFocusManager.Client {
    ...
    private static final String TAG = App.TAG + ".PlayService";
    ...
    private int _lastLoadingDirection;
    ...
    public interface PlayClient {
        ...
        void receive(Player player, Bundle bundle);
    }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 1:
                    ...
                    return;
                case 2:
                    if (PlayService.instance != null && PlayService.instance.pp != null) {
                        PlayService.instance.nextAuto(true);
                        return;
                    }
                    return;
                case 3:
    ...
    public void transferPlayer(@Nullable PlayClient owner, Player player, PlayLister lister, Intent activityIntent, Bundle savedStates) {
        if (this.pp != null) {
            throw new IllegalStateException("PP already exists.");
        }
        Log.d(TAG, "Transfer Player(" + player + ") from " + owner);
        this.pp = player;
        this._owner = owner;
        this._savedStates = savedStates;
        this._lister = lister;
        this._lastLoadingDirection = 1;
        this.pp.setClient(this);
        lister.setListener(this);
        this._intent = new Intent(activityIntent).setPackage(getPackageName()).setFlags(DriveFile.MODE_READ_WRITE);
        updateNotification(5);
        updateAudioMediaListener();
        registerReceiver(this._broadcastReceiver, this._broadcastReceivingFilter);
        updateStereoMode();
        stateHandler.sendMessageDelayed(stateHandler.obtainMessage(3, this.pp.getUri()), 500L);
    ...
    public static class ReturnedData {
        ...
        public final PlayLister lister;
        public final Player player;
        public final Bundle savedStates;

        public ReturnedData(Uri intentUri, Player player, PlayLister lister, Bundle savedStates) {
        ...
        public void close() {
            this.player.disburden();
            this.player.clear();
            this.lister.close();
        }
    }

    public ReturnedData returnPlayer(PlayClient caller) {
        if (this.pp == null || this._intent == null) {
            return null;
        }
        Log.d(TAG, "Return Player(" + this.pp + ") to " + caller + " (Original Owner: " + this._owner + ")");
        ReturnedData returnedData = new ReturnedData(this._intent.getData(), this.pp, this._lister, this._savedStates);
        this.pp.cancelRemoteLoading();
        this._lister.setListener(null);
        PlayClient owner = this._owner;
        this.pp = null;
        this._owner = null;
        ...
        if (owner != null && owner != caller) {
            Log.d(TAG, "Finishing previous owner " + owner);
            owner.finish();
        }
        try {
    ...
    private void clear(boolean finishOwner) {
        if (this.pp != null) {
            this.pp.save();
        }
        PlayClient owner = this._owner;
        ReturnedData ret = returnPlayer(finishOwner ? null : this._owner);
        if (ret != null) {
            if (finishOwner || owner == null) {
                Playlist.Autogen.cleanup(ret.intentUri);
                ret.close();
                ActivityMediaList.rebuildAll();
                return;
            }
            owner.receive(ret.player, ret.savedStates);
        }
    }
    ...
    private void updateViews(RemoteViews views, int changes, boolean bigView) {
        ...
        if (this.pp.isInPlaybackState()) {
            ...
            L.sb.insert(0, "%s / ");
            views.setChronometer(R.id.progress, SystemClock.elapsedRealtime() - this.pp.getCurrentPosition(), L.sb.toString(), this.pp.getState() == 4);
        } else {
        ...
        if (Build.VERSION.SDK_INT >= 11) {
            views.setImageViewResource(R.id.playpause, this.pp.getTargetState() == 4 ? R.drawable.ic_button_pause : R.drawable.ic_button_play);
            if (!bigView) {
                if (Build.VERSION.SDK_INT < 21) {
                    ...
                    views.setViewVisibility(R.id.prev, screenWidthDp < 500.0f ? 8 : 0);
                } else {
        ...
        if ((changes & 1) != 0) {
            ...
            setCover(views, R.id.cover, (268435456 & changes) == 0, bigView);
    ...
    */
    private void setCover(RemoteViews views, int id, boolean loadRemoteCover, boolean bigView) {
        ...
        try {
            ...
            if (this.pp.hasCover()) {
                ...
                if (loadRemoteCover) {
                    this.pp.loadRemoteCovers(this._lister);
                }
    ...
    private void loadNext(Uri uri, int direction) {
        String ext;
        Log.d(TAG, "Load Next");
        this.pp.clear(16);
        ActivityScreen.arrangeExtras(this._intent, uri);
        this._lastLoadingDirection = direction;
        byte requestedDecoder = this._intent.getByteExtra(ActivityScreen.EXTRA_DECODE_MODE, (byte) 0);
        ...
        this.pp.load(null, 0, App.prefs.getBoolean(Key.USE_SPEEDUP_TRICKS, false));
        start();
    ...
    public void previous(boolean noRestart) {
        boolean canRestart = true;
        if (this.pp.isInValidState()) {
            if (noRestart || !this.pp.isSeekable() || !App.prefs.getBoolean(Key.SMART_PREV, true) || !this.pp.isInPlaybackState()) {
                canRestart = false;
            }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public void nextAuto(boolean immediate) {
        if (!P.playerBackToList) {
            if (P.playerLooping == 1 && this.pp.getState() != -1) {
                if (immediate) {
                    if (this.pp.isSeekable()) {
                        ...
                        this.pp.clear(16);
                        this.pp.setDataSource(uri, decoder, 0, 0);
                        this.pp.load(null, 0, App.prefs.getBoolean(Key.USE_SPEEDUP_TRICKS, false));
                    }
                    start();
                    return;
                }
                stateHandler.sendMessageDelayed(stateHandler.obtainMessage(2, 1, 0), 100L);
                return;
            }
            int direction = P.shuffle ? 0 : 1;
            Uri next = this._lister.getAndUpdate(this.pp.getUri(), direction);
    ...
    public void onNetworkListingCompleted(PlayLister lister) {
        if (this.pp.isInPlaybackState()) {
            this._lister.loadRemoteAdjascentUri(this.pp.getUri());
        }
    }
    ...
    public boolean canStart() {
        if (Build.VERSION.SDK_INT >= 8) {
            ...
            return this._audioFocusManager.canStartPlayback();
        }
        return true;
    }
