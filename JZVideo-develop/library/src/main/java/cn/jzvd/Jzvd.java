...
public abstract class Jzvd extends FrameLayout implements View.OnClickListener, SeekBar.OnSeekBarChangeListener, View.OnTouchListener {
    ...
    public void init(Context context) {
        ...
        if (_supremeText == null) {
            _supremeText = new TextView(context);
        }
        if (_supremeContainer == null) {
            _supremeContainer = new View(context);
        }
...
//                    touchActionMove(x, y);
                    ...
                    return ACTION_MOVE(v, event, this._gestures);
...
//                    break;
                ...
                case 262 /* 262 */:
//                Log.e("位移值numPointer：", String.valueOf(event.getPointerCount() == 2 && (this._swiping & 16) != 0));
                    if (event.getPointerCount() == 2 && (this._swiping & 16) != 0) {
                        int erased = getActionIndex(event.getAction());
...
public boolean ACTION_MOVE(View v, MotionEvent event, int gestures){
    float height;
    float width;
    ...
    switch (this._swiping) {
        case 0:
            ...
            if (numPointer >= 2) {
                ...
                if (distance2 + distance22 >= dragMinimumPixel) {
                    ...
                    if ((gestures & 7) != 0 && mediaInterface.isPlaying()) {
                        int videoWidth = this.mediaInterface.getVideoWidth(); //this.pp.getDisplayWidth();
                        int videoHeight = this.mediaInterface.getVideoHeight(); //this.pp.getDisplayHeight();
                        if (videoWidth > 0 && videoHeight > 0) {
                            ...
                            this._panStartX = this._panX;
                            this._panStartY = this._panY;
                            this._zoomDragBeginWidth = this.textureViewContainer.getWidth();
                ...
                touchActionMove(event.getX(), event.getY());
                return true;
            }
        case 17:
        case 18:
        case 19:
            if (this.mediaInterface.isPlaying()) {
                int videoWidth2 = this.mediaInterface.getVideoWidth();//this.pp.getDisplayWidth();
                int videoHeight2 = this.mediaInterface.getVideoHeight();//this.pp.getDisplayHeight();
                ...
                if (videoWidth2 > 0 && videoHeight2 > 0) {
                    if (((this._swiping == 17) | (this._swiping == 19)) && event.getPointerCount() >= 2) {
                        ...
                        if (this._zoomWidth != ((int) width) || this._zoomHeight != ((int) height) ) {
...
//                Log.v(TAG, "onProgressUpdate " + "[" + this.hashCode() + "] ");
    ...
    public void setDisplaySize(int width, int height) {
        if (this.mediaInterface.isPlaying()) {
            if (width > 0 && height > 0) {
                RelativeLayout.LayoutParams l = (RelativeLayout.LayoutParams) this.textureViewContainer.getLayoutParams();
                l.width = width;
                l.height = height;
                int topWidth = this.textureViewContainer.getWidth();
                int topHeight = this.textureViewContainer.getHeight();
